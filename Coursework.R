...
#Age x part B score
ggplot(dataset, aes(x=Age, y=Part.B.Score)) + geom_point() + geom_smooth() + theme_bw() + xlim(0, 100) + ylim(0, 100) + labs(title="Scatterplot Showing How Part B Scores Change With Age", x="Age", y="Part B Score")
#Age x completion time x gender 
ggplot(dataset, aes(x=Age, y=Completion.Time, color=Gender)) + geom_smooth() + facet_wrap(~ Gender) + theme_bw() + xlim(0, 100) + labs(title="Line Graph showing how completion time changes with age, split by gender", x="Age", y="Completion Time")
#Age x part C score x location
ggplot(dataset, aes(x=Age, y=Part.C.Score, color=Location)) + geom_smooth() + facet_wrap(~ Location) + theme_bw() + xlim(0, 100) + labs(title="Line Graph showing how part c scores vary with age, split by location", x="Age", y="Part C Score")
#completion time x part B score x Gender
ggplot(dataset, aes(x=Completion.Time, y=Part.B.Score, color=Gender)) + geom_smooth() + facet_wrap(~ Gender) + theme_bw() + ylim(0, 100) + labs(title="Line Graph showing how completion time and part b score, split by gender", x="Completion Time", y="Part B Score")
